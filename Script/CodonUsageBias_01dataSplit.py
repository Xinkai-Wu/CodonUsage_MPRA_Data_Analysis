...
def MainInfoFeatch(constant5prime,constant3prime,read1_,read2_rc_):
    ...
    if str(read1_constant5) != 'nomatch':
        if read1_constant5 + 56 <= len(read1_): # the minmum lenth to fetch insert seq
            r1_insert_start = read1_constant5 + 20 # the length of constant5
    ...
    if str(read2_constant5) != 'nomatch':
        if read2_constant5 + 56 <= len(read2_rc_):
            r2_insert_start = read2_constant5 + 20 
...
constant5prime = 'GCGATGCCACCTACGGCAAG'
constant3prime = 'GTGACCACCCTGACCTACGG' # reverse compliment to match
